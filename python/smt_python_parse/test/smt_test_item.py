...
#%%
print("\n Example1 with Algebraic num for testing")
a:TextIOWrapper=open("../../../benchmark/test/C_3_64.smt2","r")
slib=read_smtlib2(a)
print(slib.vars)
f=smt_op(slib,0)
xd={str(x):1 for x in slib.vars}
x=list(map(lambda key:xd[str(key)],slib.vars))
print(abs(f(x)-0.0))
# assert abs(f(x)-0.0)<1.0e-6

#%%
print("\n Example2 with Algebraic num for testing")
a:TextIOWrapper=open("../../../benchmark/non-incremental/QF_NRA/2019-ezsmt/bouncingBall/inst_1_100.smt2","r")
slib=read_smtlib2(a)
f=smt_op(slib,0)
var_assigns={str(x):1 for x in slib.vars}
x=list(map(lambda f:var_assigns[str(f)],slib.vars))
# assert abs(f(x)-0.0)<0.1
#%%
# 把控制台打出来的公式展开便于验证
